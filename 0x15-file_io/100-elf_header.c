 ...
 * Description: if the the cannot be closed - exit code 98
 */
 ...
 * @argc: number of arguments suppied to the program
 * @argv: array of pointers to the arguments
 * Return: 0 on Success
 * Description: not an ELF file of function fails - exit code 98
 */
...
int main(int __attribute__((_unused_)) argc, char *argv[])
{
	...
	if (r == -1)
	{
		free(header);
		close_elf(o);
		dprinf(STDERR_FILENO, "Error: '%s': No such file\n", argv[1]);
		exit(98);
	}
